...
wav_to_matrix <- function(tmp, augment = FALSE, output_type = "01true", subfix=NULL){
  ...
  for(i in 1:length(output_type)){
    ...
    if(!is.null(subfix)){
      file_name = paste0("output/finalized/keras_frames2/", cohort, "_" , test, "_", output_type[i],"_", subfix,".rds")
    } else {
      file_name = paste0("output/finalized/keras_frames2/", cohort, "_" , test, "_", output_type[i], ".rds")
    }
    print(file_name)
