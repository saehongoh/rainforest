...
test_noaug <- data.frame(do.call(rbind, strsplit(test_noaug,"/|_|[.]"))[,4:5], file_id = test_noaug)
colnames(test_noaug)[1:2] <- c('aug','species_id')

test_aug <- list.files("output/january", pattern="_Aug", full.names = TRUE)
test_aug <- data.frame(do.call(rbind, strsplit(test_aug,"/|_|[.]"))[,4:5], file_id = test_aug)
colnames(test_aug)[1:2] <- c('aug','species_id')
...
#######################
tmp_frame <- train_prepper_species(readRDS("output/january/test_Aug_s0.RDS"), "s0")
...
for(i in 1:length(species_list)){
  ...
  test_data2 <- train_prepper_species(readRDS(test_aug$file_id[i]), species_list[i])
  ...
  print("evalulating on raw testing data")
  ...
  print(paste("raw test acc : ", round(eval1[[2]],digits=3), "aug test acc : ", round(eval2[[2]],digits=3)))
