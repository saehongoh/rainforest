...
train_key <- readRDS("output/resized_samplekey.rds") %>%
  mutate(species_id = ifelse(species_id == 17 & songtype_id == 4, species_id+0.4, species_id)) %>%
  group_by(species_id) %>%
  ...
  specmaker_v2species <- function(tmp, x, augment = FALSE, multi=1){
    ...
    if(augment == TRUE & unique(res$cate) == "true"){
      ...
      augmentation <- function(x, shift_time){
          ...
          noise <- rnorm(sum(corrupt),mean(res$value),sd(res$value)/sample(seq(2, 6, 0.5), 1))
    ...
    reSizeMe <- function(res){
      ...
      df1 <- data.frame(zts = df1$zts, ResizeMat(df1[,2:ncol(df1)], c(nrow(df1), 40)))
      ...
      df2 <- data.frame(ResizeMat(df1, c(1600, ncol(df1)))) 
      
      if(length(which(is.na(df2))) > 0){
        df2 <- df1[1:(nrow(df1)-1),]
        df2 <- data.frame(ResizeMat(df2, c(1600, ncol(df2)))) 
      }
...
#   print(paste0("starting ", species_list[i]))
#   tmp <- test_key %>%
...
#   if(length(which(is.na(res$value)))>0){stop("there are NAs in data!")}
#   res <- keras_unmasked_v2(res)
#   file_name = paste0("output/january/test_NoAug_s", species_list[i], ".RDS")
#   print(file_name)
#   saveRDS(res, file_name)
# }

#############################################################
#### Species specific test files with other species
#############################################################
...
use_cores <- ifelse(table(tmp$cate)[[1]]*2 >= 900, FALSE, TRUE)
  # multiplier = 2
res <- specmaker_v4species(tmp, augment = FALSE, multi=1, use_multicore = use_cores)
  
if(length(which(is.na(res$value)))>0){stop("there are NAs in data!")}
res <- keras_unmasked_v2(res)
file_name = paste0("output/january/test_withothers_all.RDS")
print(file_name)
saveRDS(res, file_name)
