...
# train_key %>%
#   group_by(species_id, cate) %>%
#   mutate(group_n = ceiling(seq_along(file_id)/(length(file_id)/4))) %>%
#   ungroup() %>%
#   group_by(species_id, cate,group_n) %>%
#   summarise(n())
...
repeat{
  ...
  if(length(which(is.na(res$value)))>0){stop("there are NAs in data!")}
  res <- keras_unmasked_v2(res)
  file_name = paste0("output/resized/train_posonly_aug", j, ".RDS")
  print(file_name)
  saveRDS(res, file_name)
  if(j==2){break}
  j=j+1
...
for(i in 1:length(train_batches)){
  ...
  repeat{
    ...
    if(length(which(is.na(res$value)))>0){stop("there are NAs in data!")}
    res <- keras_unmasked_v2(res)
    file_name = paste0("output/resized/train_s", unique(tmp$species_id),"_aug", j, ".RDS")
    print(file_name)
    saveRDS(res, file_name)
    if(j==2){break}
    j=j+1
