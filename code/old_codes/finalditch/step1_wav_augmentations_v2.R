...
specmaker_v2 <- function(tmp, x, augment = NA, use_augment_only = FALSE, multi=1, manual_n){
  ...
  if(augment == "true"){
    ...
    if(unique(res$cate) == "true"){
      ...
      multi=multi
      ### Horizontal shifts
      min_shift = round(max(res$zts)*.10)
      max_shift = round(max(res$zts)*.90)
      deviations = sapply(1:multi, function(x) sample(seq(min_shift, max_shift, round((max_shift-min_shift)/10)), 1))
      ### percentage of values to corrupt
      per_min = 0.1
      per_max = 0.5
      percents <- sapply(1:multi, function(x) sample(seq(per_min, per_max, (per_max - per_min)/10), 1))
      ### noise level
      min_noise = 0.20
      max_noise = 0.80
      noise <- sapply(1:multi, function(x) sample(seq(min_noise, max_noise, (max_noise - min_noise)/10), 1))
      #### vertical flip
      vert <- sapply(1:multi, function(x) sample(c(1,0), 1))
      
      augmented_data <- data.table::rbindlist(lapply(1:multi, function(x) augmentations(res, x=x, shift_time= deviations[x], percent_p=percents[x], noise_level=noise[x], verflip = vert[x])))
    ...
    ### Horizontal shifts
    min_shift = round(max(res$zts)*.10)
    max_shift = round(max(res$zts)*.50)
    deviations = sapply(1:multi, function(x) sample(seq(min_shift, max_shift, round((max_shift-min_shift)/10)), 1))
    ### percentage of values to corrupt
    per_min = 0.1
    per_max = 0.5
  ...
  df2 <- data.table::rbindlist(lapply(1:length(df2), function(x) reSizeMe(df2[[x]], zts_dim=50, freq_dim=600))) %>%
    mutate(value = scale(value, center=min(value), scale = max(value) - min(value))) %>%
    mutate(value = round(value, digits=3)) 
...
wav_to_matrix <- function(tmp, augment = FALSE, output_type = "01true", manual_n = NA, use_augment_only = FALSE, subfix=NULL){
  ...
  for(i in 1:length(output_type)){
    ...
    if(!is.null(subfix)){
      file_name = paste0("output/finalditch/cohort_t2/", cohort, "_" , test, "_", output_type[i],"_", subfix,".rds")
    } else {
      file_name = paste0("output/finalditch/cohort_t2/", cohort, "_" , test, "_", output_type[i], ".rds")
    }
    print(file_name)
...
if(args[[1]] == 1){
  ...
  #############################################################
  ### Test Set
  wav_to_matrix(cohort2_test,output_type=c("01false"), augment="false", subfix="noaug_set1")
  
} else if(args[[1]] == 2){
  
  ### Train Set 1: Positives augmented to # of false positives
  wav_to_matrix(cohort2_train, output_type=c("01false"), augment="true", use_augment_only=FALSE, subfix="aug_set1")
  
} else if(args[[1]] == 3){
  
  ### Train Set 2: subsample negatives augmented to # of false positives
  tmp <- cohort2_train %>%
    group_by(species_id) %>%
    mutate(n_true = length(which(cate == "true"))) %>%
    group_by(species_id, cate) %>%
    sample_n(., n_true)
  wav_to_matrix(tmp, output_type=c("01false"), augment="both", manual_n=2, use_augment_only=TRUE, subfix="aug_set2")
  
} else if(args[[1]] == 4){
  
  ### Train Set 3: subsample negatives augmented to # of false positives
  tmp <- cohort2_train %>%
    group_by(species_id) %>%
    mutate(n_true = length(which(cate == "true"))) %>%
    group_by(species_id, cate) %>%
    sample_n(., n_true)
  wav_to_matrix(tmp, output_type=c("01false"), augment="both", manual_n=2, use_augment_only=TRUE, subfix="aug_set3")
  
} else if(args[[1]] == 5){
  
  ### Train Set 4: subsample negatives augmented to # of false positives
  tmp <- cohort2_train %>%
    group_by(species_id) %>%
    mutate(n_true = length(which(cate == "true"))) %>%
    group_by(species_id, cate) %>%
    sample_n(., n_true)
  wav_to_matrix(tmp, output_type=c("01false"), augment="both", manual_n=2, use_augment_only=TRUE, subfix="aug_set4")
  ...
  #############################################################
  ### Test Set
  wav_to_matrix(cohort3_test,output_type=c("01false"), augment="false", subfix="noaug_set1")
  
} else if(args[[1]] == 7){
  
  ### Train Set 1: Positives augmented to # of false positives
  wav_to_matrix(cohort3_train, output_type=c("01false"), augment="true", use_augment_only=FALSE, subfix="aug_set1")
  ...
  #############################################################
  ### Test Set
  wav_to_matrix(cohort1_test,output_type=c("01false"), augment="false", subfix="noaug_set1")
  
} else if(args[[1]] == 12){
  
  ### Train Set 1: Positives augmented to # of false positives
  wav_to_matrix(cohort1_train, output_type=c("01false"), augment="true", use_augment_only=FALSE, subfix="aug_set1")
