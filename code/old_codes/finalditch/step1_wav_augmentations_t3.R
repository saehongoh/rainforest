...
specmaker_v2 <- function(tmp, x, augment = NA, use_augment_only = FALSE, multi=1, manual_n){
  ...
  if(augment == "true"){
    ...
    if(unique(res$cate) == "true"){
      ...
      howmany=multi/2
      ### Horizontal shifts
      min_shift = round(max(res$zts)*.20)
      max_shift = round(max(res$zts)*.80)
      deviations = sapply(1:howmany, function(x) sample(seq(min_shift, max_shift, round((max_shift-min_shift)/10)), 1))
      ### percentage of values to corrupt
      per_min = 0.05
      per_max = 0.25
      percents <- sapply(1:howmany, function(x) sample(seq(per_min, per_max, (per_max - per_min)/10), 1))
      ### noise level
      min_noise = 0.05
      max_noise = 0.25
      noise <- sapply(1:howmany, function(x) sample(seq(min_noise, max_noise, (max_noise - min_noise)/10), 1))
      #### vertical flip
      # vert <- sapply(1:howmany, function(x) sample(c(1,0), 1))
      ...
      max_shift = round(max(res$zts)*.80)
      deviations = sapply(1:howmany, function(x) sample(seq(min_shift, max_shift, round((max_shift-min_shift)/10)), 1))
      augmented_data2 <- data.table::rbindlist(lapply(1:howmany, function(x) augmentations(res, x=x, shift_time= deviations[x], percent_p=NA, verflip = 0))) %>%
        mutate(unique_id = paste0(unique_id, "_set2"))
      ...
      if(use_augment_only == TRUE){
        res <- rbind(augmented_data, augmented_data2)
      } else {
        res <- rbind(res, augmented_data, augmented_data2)
      }
...
wav_to_matrix <- function(tmp, augment = FALSE, output_type = "01true", manual_n = NA, use_augment_only = FALSE, subfix=NULL){
  ...
  for(i in 1:length(output_type)){
    ...
    if(!is.null(subfix)){
      file_name = paste0("output/finalditch/cohort_t3_noguas/", cohort, "_" , test, "_", output_type[i],"_", subfix,".rds")
    } else {
      file_name = paste0("output/finalditch/cohort_t3_noguas/", cohort, "_" , test, "_", output_type[i], ".rds")
    }
    print(file_name)
