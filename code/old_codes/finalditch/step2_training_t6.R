...
test_data <- train_prepper(readRDS("output/finalditch/cohort_t6/cohort2_test_01false_noaug.rds"))
# dim(train_data[[1]])
dim(test_data[[1]])
...
train_cohort <- list.files("output/finalditch/cohort_t6", pattern="cohort2", full.names = TRUE)
...
repeat{
  for(i in 1:nrow(train_cohort)){
    ...
    print(res_frame)
    file_name = paste0("output/finalditch/model_t6/cohort2_r",r,"_set",i,".h5")
    print(file_name)
