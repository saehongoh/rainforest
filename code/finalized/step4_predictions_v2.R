...
make_preds_wrapper <- function(tmp){
  ...
  make_preds <- function(tmp, x){
    ...
    trained_models <- df1[df1$cohort == paste0("cohort",x),]
    best <- trained_models %>%
      top_n(., true_pos_rate, n=1) %>%
      top_n(., -test_loss, n=1)
    
    rest <- trained_models[!(trained_models$file_id %in% best$file_id),]
    model <- keras::load_model_hdf5(best$file_id)
    i=1
    repeat{
      model %>% keras::load_model_weights_hdf5(filepath = rest$file_id[i])
      if(i == nrow(rest)){break}
      i=i+1
    }
...
write.csv(export, paste0("output/finalized/submissions/fullrun3_preds_b",args[[1]], ".csv"), row.names = F, quote = F)
write.csv(df1,paste0("output/finalized/submissions/fullrun3_models_b",args[[1]], ".csv"), row.names = F, quote = F)
...
# 
# # 
files <- list.files("output/finalized/submissions", pattern="fullrun3_preds", full.names = TRUE)
export <- data.table::rbindlist(lapply(1:length(files), function(x) data.table::fread(files[x])))
template <- read.csv("data/sample_submission.csv")
...
export <- export[match(template$recording_id, export$recording_id),]
write.csv(export, paste0("output/finalized/submissions/fullrun3_unrounded.csv"), row.names = F, quote = F)
