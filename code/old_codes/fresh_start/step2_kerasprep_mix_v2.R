...
keras_whole <- function(data){
  ...
  lister <- function(x){
    list(f1 %>% dplyr::filter(zts ==times[x])%>% dplyr::select(-unique_id, -value, -zts) %>% as.matrix(),
         f2 %>% dplyr::filter(zts ==times[x])%>% dplyr::select(-unique_id, -value, -zts) %>% as.matrix(),
         f3 %>% dplyr::filter(zts ==times[x])%>% dplyr::select(-unique_id, -value, -zts) %>% as.matrix(),
         f4 %>% dplyr::filter(zts ==times[x])%>% dplyr::select(-unique_id, -value, -zts) %>% as.matrix())
  }
  ...
  # system.time(
  # parallel::mclapply(1:10, function(x) print(x^2))
  # )
  # system.time(
    # pbmcapply::pbmclapply(1:10, function(x) print(x^2), mc.cores = 4)
  # )
  ...
  for(j in 1:length(batches)){
    ...
    keras_whole_v2 <- function(input_data){
      ...
      data_labels <- data_x[[2]] %>%
        mutate(unique_id = ifelse(grepl("aug", unique_id), paste0(unique_id, i), unique_id))
    ...
    saveRDS(keras_whole_v2(tmp), paste0("output/ind_keras/", data_list[i],"_B", j, ".RDS"))
  }
}
