...
train_prepper_1D <- function(input){
  train <- input[[1]]
  train <- array(train, dim=c(dim(train)[1:3]))
...
pos_files$batch = c(rep(1, 5), rep(2, 4))

# lapply(1:nrow(pos_files), function(x) length(which(is.na(train_prepper(readRDS(pos_files$file_id[x]))[[1]]))))
...
# lapply(1:nrow(posneg_files), function(x) length(which(is.na(train_prepper(readRDS(posneg_files$file_id[x]))[[1]]))))

posneg_files <- posneg_files[sample(1:nrow(posneg_files)),] %>%
  group_by(aug) %>%
  group_split(.) 

#### Test data for evaluation
test_data1 <- train_prepper(readRDS("output/resized/test_ori.RDS"))
test_data1 <- train_prepper_1D(readRDS("output/resized/test_ori.RDS"))
...
model %>% 
  ...
  layer_conv_1d(input_shape = c(dim(test_data1[[1]])[-1]),
                filters=32, kernel_size = c(4), activation = 'relu', kernel_regularizer = regularizer_l2(l = 0.001)) %>%
  # layer_max_pooling_1d(pool_size = 2) %>%
  # layer_global_max_pooling_1d() %>%
  # layer_max_pooling_1d(pool_size = 2) %>%
  layer_lstm(units = 64,  activation = 'relu') %>%
  # layer_conv_1d(filters=2, kernel_size = c(50), activation = 'relu') %>%
  # layer_conv_1d(filters=128, kernel_size = c(50), activation = 'relu') %>%
...
# model <- load_model_hdf5("output/resized_model/pos_tmp_Set1_aug4.h5")

##############################################
##### Train data 
#############################################
for(j in 1:length(pos_files)){
tmp_pos <- pos_files[[j]]
tmp_pos <- tmp_pos[sample(1:nrow(tmp_pos)),]

for(i in 1:nrow(tmp_pos)){
  print(paste("working on ___ ", tmp_pos$file_id[i]))
  train_data <- train_prepper_1D(readRDS(tmp_pos$file_id[i]))

  model %>% fit(train_data[[1]], train_data[[2]], batch_size = floor(nrow(train_data[[2]])/8), epochs = 100, shuffle = TRUE, 
  ...
  file_name = paste0("output/resized_lr5/pos_tmp_Set",j,"_", tmp_pos$aug[i], ".h5")
  model %>% save_model_hdf5(file_name)
}

print("saving final model")
file_name = paste0("output/resized_lr5/pos_post_Set", j ,".h5")
model %>% save_model_hdf5(file_name)

##### Train posonly data Set1
tmp_posneg <- posneg_files[[j]]

for(i in 1:nrow(tmp_posneg)){
  print(paste("working on ___ ", tmp_posneg$file_id[i]))
  train_data <- train_prepper_1D(readRDS(tmp_posneg$file_id[i]))
  
  model %>% fit(train_data[[1]], train_data[[2]], batch_size = floor(nrow(train_data[[2]])/12), epochs = 100, shuffle = TRUE, 
                validation_split=0.15,
                verbose=1,
  ...
  print("saving ongoing model")
  file_name = paste0("output/resized_lr5/posneg_tmp_Set",j,"_", tmp_posneg$batch[i], ".h5")
  model %>% save_model_hdf5(file_name)
}

print("saving final model")
file_name = paste0("output/resized_lr5/posneg_post_Set", j ,".h5")
model %>% save_model_hdf5(file_name)
}

##############################################
##### Train posonly data Set2
#############################################
