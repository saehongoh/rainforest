...
test_cohort <- list.files("output/finalditch/cohort", pattern="test", full.names = TRUE)
test_cohort <- data.frame(do.call(rbind, strsplit(test_cohort,"/|_|[.]rds"))[,4:7], file_id = test_cohort)
colnames(test_cohort)[1:4] <- c('cohort','test','type','aug')
...
#### test files with other species by cohort
train_cohort <- list.files("output/finalditch/cohort", pattern="train", full.names = TRUE)
train_cohort <- data.frame(do.call(rbind, strsplit(train_cohort,"/|_|[.]rds"))[,4:7], file_id = train_cohort)
colnames(train_cohort)[1:4] <- c('cohort','test','type','aug')
train_cohort$activation <- ifelse(grepl("true|false", train_cohort$type), "softmax", "sigmoid")

#######################
print("cohort-wise training")
#######################
train_tmp <- train_cohort %>% filter(aug == "aug") %>% filter(!grepl("set", file_id))

i=3
train_tmp[i,]
test_cohort[i,]
train_data <- train_prepper(readRDS(train_tmp$file_id[i]))
test_data <- train_prepper(readRDS(test_cohort$file_id[i]))
dim(train_data[[1]])
dim(test_data[[1]])
head(train_data[[2]])
head(test_data[[2]])
...
model %>% 
  ...
  layer_dense(units = dim(test_data[[2]])[2]*2, activation = "relu", kernel_regularizer = regularizer_l2(l = 0.01)) %>%
...
# file_name = paste0("output/finalditch/models/cohort2_post_set1_run1.h5")
