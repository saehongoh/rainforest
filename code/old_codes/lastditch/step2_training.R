...
#### test files
test_files <- list.files("output/lastditch", pattern="test", full.names = TRUE)
test_files <- data.frame(do.call(rbind, strsplit(test_files,"/|_|[.]RDS"))[,3:5], file_id = test_files)
colnames(test_files)[1:3] <- c('cohort','test','type')
test_files$activation <- ifelse(grepl("true|false", test_files$type), "softmax", "sigmoid")

#### train files
train_files <- list.files("output/lastditch", pattern="train", full.names = TRUE)
train_files <- data.frame(do.call(rbind, strsplit(train_files,"/|_|[.]RDS"))[,3:6], file_id = train_files)
colnames(train_files)[1:4] <- c('cohort','test','type','n')
train_files <- train_files %>% filter(n== "n20")
train_files$activation <- ifelse(grepl("true|false", train_files$type), "softmax", "sigmoid")

i=3
train_files[i,]
test_data <- train_prepper(readRDS(test_files$file_id[i]))
train_data <- train_prepper(readRDS(train_files$file_id[i]))
dim(test_data[[1]])
dim(train_data[[1]])
...
model %>% 
  layer_conv_2d(input_shape = c(dim(test_data[[1]])[-1]),
                filters = 32, kernel_size = c(3,3), activation = 'relu', kernel_regularizer = regularizer_l2(l = 0.01)) %>%
  layer_conv_2d(filters = 32, kernel_size = c(3,3), activation = 'relu', kernel_regularizer = regularizer_l2(l = 0.01)) %>%
  layer_spatial_dropout_2d(rate=0.15) %>%
  layer_max_pooling_2d(pool_size = c(2,2)) %>%
  layer_conv_2d(filters = 64, kernel_size = c(3,3), activation = 'relu', kernel_regularizer = regularizer_l2(l = 0.01)) %>%
  layer_conv_2d(filters = 64, kernel_size = c(3,3), activation = 'relu', kernel_regularizer = regularizer_l2(l = 0.01)) %>%
  layer_spatial_dropout_2d(rate=0.15) %>%
  layer_max_pooling_2d(pool_size = c(2,2)) %>%
  layer_conv_2d(filters = 128, kernel_size = c(3,3), activation = 'relu', kernel_regularizer = regularizer_l2(l = 0.01)) %>%
  layer_conv_2d(filters = 128, kernel_size = c(3,3), activation = 'relu', kernel_regularizer = regularizer_l2(l = 0.01)) %>%
  layer_spatial_dropout_2d(rate=0.15) %>%
  layer_max_pooling_2d(pool_size = c(2,2)) %>%
  ...
  # layer_flatten() %>%
  layer_dense(units = dim(test_data[[2]])[2], 
              activation = test_files$activation[i],
