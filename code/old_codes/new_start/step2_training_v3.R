...
model %>% 
  ...
  layer_dense(units = 12, activation = "relu", kernel_regularizer = regularizer_l2(l = 0.01)) %>%
  layer_dropout(rate = 0.60) %>%
  layer_dense(units = 12, activation = "relu", kernel_regularizer = regularizer_l2(l = 0.001)) %>%
  layer_dropout(rate = 0.50) %>%
  layer_dense(units = 6, activation = "relu", kernel_regularizer = regularizer_l2(l = 0.001)) %>%
  layer_dropout(rate = 0.50) %>%
  layer_dense(units = 6, activation = "relu", kernel_regularizer = regularizer_l2(l = 0.001)) %>%
  layer_dropout(rate = 0.50) %>%
...
for(i in 1:nrow(tmp_pos)){
  ...
  train_data <- train_prepper(readRDS(tmp_pos$file_id[i]))
...
for(i in 1:nrow(tmp_posneg)){
  ...
  train_data <- train_prepper(readRDS(tmp_posneg$file_id[i]))
