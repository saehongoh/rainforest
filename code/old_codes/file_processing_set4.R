...
for(i in 1:nrow(train_tp)){
  if(train_tp$duration[i] >= 1.5 & train_tp$duration[i] < 2.5){
    multiplier = 3
  } else if((train_tp$duration[i]) >= 2.5){
    multiplier = 4
  } else if((train_tp$duration[i] < 1.5)){
    multiplier = 2.5
  }
  print(paste(i, " ______ ", train_tp$duration[i], " ______ ", multiplier))
}
...
for(j in 1:length(species_list)){
  ...
  saveRDS(key, paste0("output/processed_data_set4/train_species_", species_list[j], "_key.RDS"))
  }
