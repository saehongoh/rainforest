...
pre_trained <- dplyr::case_when(args[[1]] == "cohort1" ~ "output/finalized/trained_models2/cohort1_r2_set10.h5",
                                args[[1]] == "cohort2" ~ "output/finalized/trained_models2/cohort2_r2_set10.h5",
                                args[[1]] == "cohort3" ~ "output/finalized/trained_models2/cohort3_r2_set10.h5")

print(pre_trained)

model <- keras::load_model_hdf5(pre_trained)

# add_model <- model$output %>%
#   layer_dropout(rate = 0.7)  %>%
#   layer_dense(units = dim(test_data[[2]])[2], activation = "softmax",  kernel_regularizer = regularizer_l2(l = 0.01))
# 
# keras_model(inputs = model, outputs = add_model)
...
data.table::fwrite(tmp_frame, paste0("output/finalized/",args[[1]], "_traininglog_p2.txt"), sep="\t", row.names = FALSE, quote = FALSE)
...
repeat{
  for(i in 1:length(train_files)){
    ...
    tmp <- data.table::fread(paste0("output/finalized/",args[[1]],"_traininglog_p2.txt"))
    tmp <- rbind(tmp, res_frame)
    data.table::fwrite(tmp, paste0("output/finalized/",args[[1]],"_traininglog_p2.txt"), sep="\t", row.names = FALSE, quote = FALSE)
    }
  if(r==3){break}
  r=r+1
}
...
# ### RETURN LAST TIME TO TRAIN DATA
# 
...
# train_files <- files[!(grepl("test", files))]
# 
# # model <- load_model_hdf5("output/finalized/trained_models/cohort3_r4_set1.h5")
# model$output %>%
