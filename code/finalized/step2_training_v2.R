...
files <- list.files("output/finalized/keras_frames2", pattern=args[[1]], full.names = TRUE)
...
print(paste(neurons1, neurons2, neurons3, sep = " --------- "))
#######################
...
tmp_frame <- data.frame(timestamp = format(Sys.time(),format='%H:%M'), 
                        cohort=args[[1]], round= 0, set = 0, test_acc= 0, true_pos_rate =0, false_pos_rate =  0)
data.table::fwrite(tmp_frame, paste0("output/finalized/",args[[1]], "_traininglog.txt"), sep="\t", row.names = FALSE, quote = FALSE)
...
repeat{
  for(i in 1:length(train_files)){
    ...
    model %>% fit(train_data[[1]], train_data[[2]], batch_size = floor(nrow(train_data[[2]])/8), epochs = 50, shuffle = TRUE, 
    ...
    tmp <- data.table::fread(paste0("output/finalized/",args[[1]],"_traininglog.txt"))
    tmp <- rbind(tmp, res_frame)
    data.table::fwrite(tmp, paste0("output/finalized/",args[[1]],"_traininglog.txt"), sep="\t", row.names = FALSE, quote = FALSE)
    }
  if(r==2){break}
  r=r+1
}
...
# 
# # ###### Check
# trained_model <- keras::load_model_hdf5("output/finalized/trained_models2/cohort2_r1_set10.h5")
