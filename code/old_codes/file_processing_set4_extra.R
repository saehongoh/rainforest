...
train_fp <- read.csv("data/train_fp.csv") %>% mutate(cate = "false_positive") %>%
  ...
  ungroup()  %>%
  mutate(species_id = ifelse(species_id == 13, 13.4, species_id)) %>%
  mutate(species_id = ifelse(species_id == 5, 5.4, species_id)) 
...
### Mutating some lone positive samples
train_tp <- train_tp %>%
  mutate(t_min = ifelse(species_id == 5 & duration != median(duration), t_min + (duration-median(duration)), t_min)) %>%
  mutate(duration = ifelse(species_id == 5 & duration != median(duration), median(duration), duration))  %>%
  mutate(species_id = ifelse(species_id == 5, 5.4, species_id)) %>%
  mutate(t_max = ifelse(species_id == 13 & duration != median(duration), t_max - (duration-median(duration)), t_max)) %>%
  mutate(duration = ifelse(species_id == 13 & duration != median(duration), median(duration), duration)) %>%
...
species_list <- species_list[c(6, 14)]
# species_list <- species_list[6:length(species_list)]
...
for(j in 1:length(species_list)){
  ...
  if(all(max(c(positives$duration, negatives$duration)) == c(positives$duration, negatives$duration))){
    ...
    mean_duration <- ifelse(mean_duration < 1, mean_duration + (mean_duration*0.08), mean_duration)
  ...
  for(i in 1:length(res_list)){
    ...
    if(tmp_key$cate == "true_positive"){
      ...
      p2 <- res %>% filter(time >= (tmp_key$t_min-tmp_key$duration*0.10) & time <= (tmp_key$t_max-tmp_key$duration*0.10)) %>%
      ...
      p6 <- res %>% filter(time >= (tmp_key$t_min+tmp_key$duration*0.10) & time <= (tmp_key$t_max+tmp_key$duration*0.10)) %>%
  ...
  saveRDS(res_list, paste0("output/processed_data_set4/train_species_", species_list[j], "_data.RDS"))
  saveRDS(key, paste0("output/processed_data_set4/train_species_", species_list[j], "_res.RDS"))
}
