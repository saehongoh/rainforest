...
pre_trained <- dplyr::case_when(args[[1]] == "cohort1" ~ "output/finalized/trained_models2/cohort1_r4_set10.h5",
                                args[[1]] == "cohort2" ~ "output/finalized/trained_models2/cohort2_r4_set10.h5",
                                args[[1]] == "cohort3" ~ "output/finalized/trained_models2/cohort3_r4_set10.h5")
...
data.table::fwrite(tmp_frame, paste0("output/finalized/",args[[1]], "_traininglog_p4.txt"), sep="\t", row.names = FALSE, quote = FALSE)
...
repeat{
  for(i in 1:length(train_files)){
    ...
    tmp <- data.table::fread(paste0("output/finalized/",args[[1]],"_traininglog_p4.txt"))
    tmp <- rbind(tmp, res_frame)
    data.table::fwrite(tmp, paste0("output/finalized/",args[[1]],"_traininglog_p4.txt"), sep="\t", row.names = FALSE, quote = FALSE)
    }
  if(r==5){break}
  r=r+1
}
