...
specmaker_v4species <- function(tmp, augment = FALSE, use_multicore = TRUE, multi=1, add_sd=FALSE){
  
  specmaker_v2species <- function(tmp, x, augment = FALSE, multi=1, add_sd=FALSE){
    ...
    if(augment == TRUE & unique(res$cate) == "true"){
      ...
      augmentation <- function(x){
        if(unique(res$t_min) > 30){
          shift_time = -sample(seq(3,27,3),1)
          add_time = 60
      ...
      res <- data.table::rbindlist(lapply(1:multi, function(x) augmentation(x)))
    ...
    if(add_sd == TRUE){
      sd_tmp <- unique(round(seq(sd(res$value)/100, sd(res$value)/10, 0.0001), digits=3))
      res$value = res$value + sapply(1:nrow(res), function(x) sample(sd_tmp, 1))
    }
  ...
  if(use_multicore == TRUE){
    df1 <- pbmcapply::pbmclapply(1:nrow(tmp), function(x) specmaker_v2species(tmp, x, augment=augment, multi=multi, add_sd=add_sd), mc.cores=4)
  } else {
    df1 <- lapply(1:nrow(tmp), function(x) specmaker_v2species(tmp, x, augment=augment, multi=multi, add_sd=add_sd))
  }
...
#   # multiplier = 2
#   
#   res <- specmaker_v4species(tmp, augment = TRUE, add_sd = FALSE, multi=multiplier, use_multicore = use_cores)
#   
#   if(length(which(is.na(res$value)))>0){stop("there are NAs in data!")}
#   res <- keras_unmasked_v2(res)
#   file_name = paste0("output/january/test_Aug_s", species_list[i], ".RDS")
#   print(file_name)
#   saveRDS(res, file_name)
# }
...
train_batches <-  train_key %>%
  group_by(species_id) %>%
  group_split(.)
# train_batches <- train_batches[2:length(train_batches)]
...
for(i in 1:length(train_batches)){
  ...
  add_tmp2 <- train_key %>%
    ...
    sample_n(., 2) %>%
    mutate(cate = "false") %>%
    mutate(unique_id = paste0(unique_id, "_b3"))
  
  tmp <- rbind(tmp, add_tmp, add_tmp2)

  multiplier = floor(table(tmp$cate)[[1]]/table(tmp$cate)[[2]])
  use_cores <- ifelse(table(tmp$cate)[[1]]*2 >= 900, FALSE, TRUE)
  
  res <- specmaker_v4species(tmp, augment = TRUE, add_sd = TRUE, multi=multiplier, use_multicore = use_cores)
