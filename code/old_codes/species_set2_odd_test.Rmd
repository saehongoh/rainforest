...
odd <- readRDS("output/odds/train_species_13_data.RDS") %>%
...
toplot <- c(unique(odds$unique_id), sample(unique(negs$unique_id), 6), sample(unique(pos$unique_id), 6))
...
train_tp %>%
  # filter(grepl("neu", unique_id)) %>%
  filter(species_id == "17") %>%
  select(recording_id, songtype_id) %>%
  dcast(recording_id ~ songtype_id) 

table(train_tp$songtype_id, train_tp$species_id)
