...
# require(pbmcapply)
export_path <- ("/Volumes/R2D2/Users/eoh/Documents/R_projects/rainforest/")
...
specmaker_lean <- function(tmp, x, augment = FALSE){
  ...
  df2 <- data.table::rbindlist(lapply(1:length(df2), function(x) reSizeMe(df2[[x]], zts_dim=384, freq_dim=64))) 
...
wav_to_matrix <- function(tmp, augment = FALSE, output_type = "01true", subfix=NULL){
  ...
  for(i in 1:length(output_type)){
    ...
    if(!is.null(subfix)){
      file_name = paste0(export_path, "output/finalized/keras_raw/", cohort, "_" , test, "_", output_type[i],"_", subfix,".rds")
    } else {
      file_name = paste0(export_path, "output/finalized/keras_raw/", cohort, "_" , test, "_", output_type[i], ".rds")
    }
    print(file_name)
...
#############################################################
# 
cohort1_train <- readRDS("output/finalized/cohort1_key.rds") %>% ungroup()
# # cohort1_test <- cohort1 %>% dplyr::filter(test == "yes")
# # cohort1_train <- cohort1 %>% dplyr::filter(test == "no")
# # 
cohort2_train <- readRDS("output/finalized/cohort2_key.rds") %>% ungroup()
# # cohort2_test <- cohort2 %>% dplyr::filter(test == "yes")
# # cohort2_train <- cohort2 %>% dplyr::filter(test == "no")
# # 
cohort3_train <- readRDS("output/finalized/cohort3_key.rds") %>% ungroup()
...
#   }
# 
# cohort1_train <- data.table::rbindlist(lapply(1:10, function(x) train_batch_maker(cohort1, x))) %>%
#   group_by(tmp_id) %>%
#   arrange(tmp_id) %>%
...
#   group_split(.)
# saveRDS(cohort1_train, "output/finalized/cohort1_key_augmented_all.rds")
# 
# cohort2_train <- data.table::rbindlist(lapply(1:10, function(x) train_batch_maker(cohort2, x))) %>%
#   group_by(tmp_id) %>%
#   arrange(tmp_id) %>%
#   mutate(random_group = sample(1:n())) %>%
#   ungroup() %>%
#   group_by(random_group) %>%
#   group_split(.)
# saveRDS(cohort2_train, "output/finalized/cohort2_key_augmented_all.rds")
# 
# cohort3_train <- data.table::rbindlist(lapply(1:10, function(x) train_batch_maker(cohort3, x))) %>%
#   group_by(tmp_id) %>%
#   arrange(tmp_id) %>%
#   mutate(random_group = sample(1:n())) %>%
#   ungroup() %>%
#   group_by(random_group) %>%
#   group_split(.)
# saveRDS(cohort3_train, "output/finalized/cohort3_key_augmented_all.rds")

# cohort1_train <- readRDS("output/finalized/cohort1_key_augmented_all.rds")
# cohort2_train <- readRDS("output/finalized/cohort2_key_augmented_all.rds")
# cohort3_train <- readRDS("output/finalized/cohort3_key_augmented_all.rds")
...
if(args[[1]] == "cohort1"){
  ...
  if(args[[2]] == 1){
    wav_to_matrix(cohort1_test, output_type=c("01false"), augment=FALSE, subfix="noaugraw")
  } else if(args[[2]] == 2){
    # cohort1_train <- cohort1_train[1:10,]
    wav_to_matrix(cohort1_train, output_type=c("01false"), augment=FALSE, subfix="noaugraw")
  }
  }
...
if(args[[1]] == "cohort2"){
  ...
  if(args[[2]] == 1){
    wav_to_matrix(cohort2_test, output_type=c("01false"), augment=FALSE, subfix="noaugraw")
  } else if(args[[2]] == 2){
    wav_to_matrix(cohort2_train, output_type=c("01false"), augment=FALSE, subfix="noaugraw")
  } 
}
...
if(args[[1]] == "cohort3"){
  ...
  if(args[[2]] == 1){
    wav_to_matrix(cohort3_test, output_type=c("01false"), augment=FALSE, subfix="noaugraw")
  } else if(args[[2]] == 2){
    wav_to_matrix(cohort3_train, output_type=c("01false"), augment=FALSE, subfix="noaugraw")
  }
}
