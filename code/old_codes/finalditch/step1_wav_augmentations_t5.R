...
specmaker_lean <- function(tmp, x, augment = FALSE){
  ...
  if(augment == TRUE){
    ...
    res <- augmentations(res, x=x, shift_time = round(max(res$zts)*unique(res$frame_shift), digit=0)) %>%
      select(-frame_shift)
  ...
  res <- reSizeMe(res, zts_dim=256, freq_dim=64)
  
  if(length(which(is.na(res$value)))>0){
    if(unique(res$cate) == "false"){
      res$value <- ifelse(is.na(res$value), 0, res$value)
      print("had to fill in nas for false")
    } else if(unique(res$cate) == "true"){
...
wav_to_matrix <- function(tmp, augment = FALSE, output_type = "01true", subfix=NULL){
  ...
  for(i in 1:length(output_type)){
    ...
    if(!is.null(subfix)){
      file_name = paste0("output/finalditch/cohort_t5/", cohort, "_" , test, "_", output_type[i],"_", subfix,".rds")
    } else {
      file_name = paste0("output/finalditch/cohort_t5/", cohort, "_" , test, "_", output_type[i], ".rds")
    }
    print(file_name)
...
cohort2_train <- data.table::rbindlist(lapply(1:length(frame_shifts), function(x) randomizer(cohort2_train, x))) %>%
  group_by(tmp_id) %>%
  arrange(tmp_id) %>%
...
saveRDS(cohort2_train, "output/finalditch/cohort2_key_augmented.rds")
#############################################################
#### training and testing by cohort
#############################################################
