...
model %>% 
  ...
  layer_conv_1d(input_shape = c(dim(test_data1[[1]])[-1]),
                filters=128, kernel_size = c(4), activation = 'relu', kernel_regularizer = regularizer_l2(l = 0.001)) %>%
  ...
  layer_dense(units = 72, activation = "relu", kernel_regularizer = regularizer_l2(l = 0.001)) %>%
