...
export_path <- ("/Volumes/Macintosh HD/Users/admin/Dropbox/rainforest/")

setwd("/Volumes/2TB/gitRepos/rainforest/")
source("code/fns.R")
...
keras_whole <- function(data){
  ...
  train <- pbmcapply::pbmclapply(1:length(times), function(x) lister(x), mc.cores = 6)
...
specmaker_lean <- function(tmp, x, augment = FALSE){
  ...
  freq_d = ifelse(unique(res$cohort) == "cohort2", 96, 64)
  df2 <- data.table::rbindlist(lapply(1:length(df2), function(x) reSizeMe(df2[[x]], zts_dim=384, freq_dim=freq_d))) 
...
specmaker_v4 <- function(tmp, augment = NA, use_multicore = TRUE){
  ...
  if(use_multicore == TRUE){
    df1 <- pbmcapply::pbmclapply(1:nrow(tmp), function(x) specmaker_lean(tmp, x, augment=augment), mc.cores=6)
  } else {
...
wav_to_matrix <- function(tmp, augment = FALSE, output_type = "01true", subfix=NULL){
  ...
  for(i in 1:length(output_type)){
    ...
    if(!is.null(subfix)){
      file_name = paste0("output/finalized/keras_frames/", cohort, "_" , test, "_", output_type[i],"_", subfix,".rds")
    } else {
      file_name = paste0("output/finalized/keras_frames/", cohort, "_" , test, "_", output_type[i], ".rds")
    }
    print(file_name)
...
cohort1_test <- data.table::rbindlist(lapply(1:10, function(x) train_batch_maker(cohort1_test, x)))
cohort2_test <- data.table::rbindlist(lapply(1:10, function(x) train_batch_maker(cohort2_test, x)))
cohort3_test <- data.table::rbindlist(lapply(1:10, function(x) train_batch_maker(cohort3_test, x)))

wav_to_matrix(cohort1_test, output_type=c("01false"), augment=FALSE, subfix="noaug_totrain")
wav_to_matrix(cohort2_test, output_type=c("01false"), augment=FALSE, subfix="noaug_totrain")
wav_to_matrix(cohort3_test, output_type=c("01false"), augment=FALSE, subfix="noaug_totrain")
...
if(args[[1]] == "cohort1"){
  ...
  if(args[[2]] == 1){
    ...
    i=1
    tmp <- cohort1_train[[i]]
    tmp1 <- tmp %>% group_by(species_id, cate) %>% sample_n(., n()/2) %>% ungroup()
    tmp2 <- tmp[!(tmp$unique_id %in% tmp1$unique_id),]
    wav_to_matrix(tmp1, output_type=c("01false"), augment=TRUE, subfix=paste0("b1_augset",i))
    wav_to_matrix(tmp2, output_type=c("01false"), augment=TRUE, subfix=paste0("b2_augset",i))
  } else if(args[[2]] == 3){
    i=2
    tmp <- cohort1_train[[i]]
    tmp1 <- tmp %>% group_by(species_id, cate) %>% sample_n(., n()/2) %>% ungroup()
...
if(args[[1]] == "cohort3"){
  ...
  if(args[[2]] == 1){
    ...
    tmp1 <- tmp %>% group_by(species_id, cate) %>% sample_n(., n()/2) %>% ungroup()
    tmp2 <- tmp[!(tmp$unique_id %in% tmp1$unique_id),]
    wav_to_matrix(tmp1, output_type=c("01false"), augment=TRUE, subfix=paste0("augset",i,"_b1"))
    wav_to_matrix(tmp2, output_type=c("01false"), augment=TRUE, subfix=paste0("augset",i,"_b2"))
    } else if(args[[2]] == 3){
    i=2
    tmp <- cohort3_train[[i]]
    tmp1 <- tmp %>% group_by(species_id, cate) %>% sample_n(., n()/2) %>% ungroup()
