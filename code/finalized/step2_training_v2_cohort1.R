...
neurons1 <- dplyr::case_when(args[[1]] == "cohort1" ~ 180,
                             args[[1]] == "cohort2" ~ 60,
                             args[[1]] == "cohort3" ~ 180)
neurons2 <- dplyr::case_when(args[[1]] == "cohort1" ~ 90,
                             args[[1]] == "cohort2" ~ 60,
                             args[[1]] == "cohort3" ~ 90)
neurons3 <- dplyr::case_when(args[[1]] == "cohort1" ~ 90,
                             args[[1]] == "cohort2" ~ 60,
                             args[[1]] == "cohort3" ~ 90)
...
#   )
pre_trained <- dplyr::case_when(args[[1]] == "cohort1" ~ "output/finalized/trained_models2/cohort1_r2_set10.h5",
...
data.table::fwrite(tmp_frame, paste0("output/finalized/",args[[1]], "_tmpres2.txt"), sep="\t", row.names = FALSE, quote = FALSE)

r=1
repeat{
  for(i in 1:length(train_files)){
    print(paste0(args[[1]], " : round : ", r+2, " : set : ", i," : training"))
    ...
    res_frame <- data.frame(cohort = args[[1]], round = r+2, set = i, 
    ...
    print(paste0("round : ", r+2, " : set : ", i," : current progress"))
    print(res_frame)
    file_name = paste0("output/finalized/trained_models2/",args[[1]], "_r",r+2,"_set",i,".h5")
    print(file_name)
    model %>% save_model_hdf5(file_name)
    
    tmp <- data.table::fread(paste0("output/finalized/",args[[1]],"_tmpres2.txt"))
    tmp <- rbind(tmp, res_frame)
    data.table::fwrite(tmp, paste0("output/finalized/",args[[1]],"_tmpres2.txt"), sep="\t", row.names = FALSE, quote = FALSE)
    }
  if(r==1){break}
  r=r+1
}
