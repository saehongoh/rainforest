...
pre_trained <- dplyr::case_when(args[[1]] == "cohort1" ~ "output/finalized/trained_models2/cohort1_r5_set10.h5",
                                args[[1]] == "cohort2" ~ "output/finalized/trained_models2/cohort2_r5_set10.h5",
                                args[[1]] == "cohort3" ~ "output/finalized/trained_models2/cohort3_r5_set10.h5")
...
model %>%
  ...
  layer_dense(units = neurons2, activation = "relu", 
              activity_regularizer = regularizer_l2(l=0.001),
              # bias_regularizer = regularizer_l2(l=0.01),
              kernel_regularizer = regularizer_l2(l = 0.001)) %>%
  layer_dropout(rate = 0.50) %>%
  layer_dense(units = neurons3, activation = "relu", 
              activity_regularizer = regularizer_l2(l=0.001),
              # bias_regularizer = regularizer_l2(l=0.01),
              kernel_regularizer = regularizer_l2(l = 0.001)) %>%
  layer_dropout(rate = 0.50) %>%
  ...
  layer_flatten() %>%
  layer_dense(units = dim(test_data[[2]])[2], activation = "softmax", 
              activity_regularizer = regularizer_l2(l=0.01), 
              # bias_regularizer = regularizer_l2(l=0.01),
              kernel_regularizer = regularizer_l2(l = 0.01)) 
...
data.table::fwrite(tmp_frame, paste0("output/finalized/",args[[1]], "_traininglog_p5.txt"), sep="\t", row.names = FALSE, quote = FALSE)
...
repeat{
  for(i in 1:length(train_files)){
    ...
    model %>% fit(train_data[[1]], train_data[[2]], batch_size = floor(nrow(train_data[[2]])/5), epochs = 75, shuffle = TRUE, 
    ...
    tmp <- data.table::fread(paste0("output/finalized/",args[[1]],"_traininglog_p5.txt"))
    tmp <- rbind(tmp, res_frame)
    data.table::fwrite(tmp, paste0("output/finalized/",args[[1]],"_traininglog_p5.txt"), sep="\t", row.names = FALSE, quote = FALSE)
    }
  if(r==7){break}
  r=r+1
}
