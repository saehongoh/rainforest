...
model %>% 
  ...
  # layer_flatten() %>%
  # layer_dense(input_shape = c(dim(real_data[[1]])[-1]),
...
for(j in 1:length(train_masked)){
  ...
  for(i in 1:nrow(unmask_key)){
    ...
    # model <- load_model_hdf5("output/unmasked_model/maskedunmasked.h5")
    print("evaluating on real data")
    model %>% evaluate(real_data[[1]], real_data[[2]])
    # model %>% predict_proba(real_data[[1]]) %>% round(., digits=3)
    ...
    file_name = paste0("output/unmasked_model/maskedunmasked.h5")
...
#   
#   species_key <- species_X[[j]]
# 
#   for(i in 1:nrow(unmask_key)){
#     
#     #### unmasked
#     print(paste("working on ___ ", j, species_key$file_id[i]))
#     train_data <- train_prepper(readRDS(species_key$file_id[i]))
#     
#     # patience_es = ifelse(j >= 5, 5, j)
#     
#     model %>% fit(train_data[[1]], train_data[[2]], batch_size = 10, epochs = 50, shuffle = TRUE, 
...
#     
#     # model %>% predict_proba(train_data[[1]]) %>% round(., digits=3)
#     test_data <- train_prepper(readRDS(species_Y$file_id[i]))
#     
#     print("evaluating on test set")
...
#     
#     print("saving ongoing model")
#     file_name = paste0("output/unmasked_model/species.h5")
#     model %>% save_model_hdf5(file_name)
#     
#   }
#   print("saving batch model")
#   file_name = paste0("output/unmasked_model/species.h5_",j,".h5")
#   model %>% save_model_hdf5(file_name)
# }
